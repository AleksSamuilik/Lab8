...
public class GGG {
    ...
    ***********************************************************************************************
    ...
    public static void main(String[] args) throws Exception {
        ...
        GGG ggg = new GGG();
        FileReader reader = new FileReader(ggg.file);
        for (int i = 0; i < ggg.file.length(); i++) {
            char letter = (char) Character.toLowerCase(reader.read());
            if (Character.isLetter(letter)) {
                if (ggg.checksArray(array, letter)) {
                    ggg.addsCountArray(array, letter);
                } else {
                    array = ggg.addsNewLetter(array, letter);
                }
            }
        }

        ggg.arraysSort(array);
        for (int i = 0; i < array.length; i++) {
